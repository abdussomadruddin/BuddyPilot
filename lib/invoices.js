...
function onboardingChecks(client) {
  ...
  const hasContact = Boolean(String(client.email || "").trim() || String(client.phone || "").trim());
  return {
    details: Boolean(
...
function assertOnboardingDetails(client) {
  ...
  if (!String(client.email || "").trim() && !String(client.phone || "").trim()) {
    throw new Error("Isi sekurang-kurangnya emel atau nombor telefon.");
  }
...
function renderReportPdf(report) {
  return new Promise((resolve, reject) => {
    ...
    const metrics = [
      ...
      [report.resultLabel === "Leads" ? "COST PER LEAD (CPL)" : "COST PER RESULT", reportMoney(report.costPerLead, report.currency), red],
    ];
    ...
    doc.fontSize(12).fillColor(muted).text(report.resultLabel === "Leads" ? `CPL ${reportMoney(report.costPerLead, report.currency)}` : `${reportMoney(report.costPerLead, report.currency)} / result`, rightX + 12, 449, { width: colW / 2 - 20, align: "center" });
