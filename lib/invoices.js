...
const { weeklyReportWhatsappMessage } = require("./report-whatsapp");
const { getInvoiceConfig } = require("./invoice-config");
const { normalizeAdsReportConfig } = require("./adflow-ads");
const {
...
const AGENCY_STATUSES = new Set(["onboarding", "active", "paused", "completed", "archived"]);

function normalizeAgencyStatus(value, client = {}) {
  const status = String(value || "").trim().toLowerCase();
  if (status) {
    if (!AGENCY_STATUSES.has(status)) throw new Error("Status agency tidak sah.");
    return status;
  }
  if (client.deletedAt || client.deleted_at || client.archivedAt || client.archived_at) return "archived";
  if (String(client.onboardingStatus || client.onboarding_status || "") === "in_progress") return "onboarding";
  if (String(client.serviceStatus || client.service_status || "") === "paused") return "paused";
  return "active";
}

function normalizeAgencyDate(value, required = false) {
...
function validateAgencyEmail(value, required = false) {
  const email = String(value || "").trim();
  if (!email && !required) return "";
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new Error("Emel client tidak sah.");
  return email;
}
...
function normalizeClient(client) {
  ...
  const adBudget = Number(client.monthlyAdBudget || client.monthly_ad_budget || 0);
  if (!Number.isFinite(amount) || amount < 0) {
    throw new Error(`Monthly retainer tidak sah untuk ${code}.`);
  }
  if (!Number.isFinite(adBudget) || adBudget < 0) {
    throw new Error(`Monthly advertising budget tidak sah untuk ${code}.`);
  }

  const agencyStatus = normalizeAgencyStatus(client.agencyStatus || client.agency_status, client);
  ...
  return {
    ...
    serviceType: String(client.serviceType || client.service_type || "").trim(),
    monthlyAdBudget: adBudget,
    startDate: normalizeAgencyDate(client.startDate || client.start_date),
    agencyStatus,
    notes: String(client.notes || "").trim(),
    archivedAt: String(client.archivedAt || client.archived_at || "").trim(),
    ...
    onboardingStatus: String(client.onboardingStatus || client.onboarding_status || "completed").trim() || "completed",
    onboardingStep: String(client.onboardingStep || client.onboarding_step || "").trim(),
    onboardingState: client.onboardingState && typeof client.onboardingState === "object"
      ? client.onboardingState
      : (client.onboarding_state && typeof client.onboarding_state === "object" ? client.onboarding_state : {}),
    onboardingCompletedAt: String(client.onboardingCompletedAt || client.onboarding_completed_at || "").trim(),
    ...
    deletedAt: String(client.deletedAt || client.deleted_at || "").trim(),
    deletedDriveFolderId: String(client.deletedDriveFolderId || client.deleted_drive_folder_id || "").trim(),
    deletedDriveFolderName: String(client.deletedDriveFolderName || client.deleted_drive_folder_name || "").trim(),
    metadata: client.metadata && typeof client.metadata === "object" ? client.metadata : {},
  };
}
...
function activeClients(clients) {
  return (clients || []).filter((client) => (
    !client.deletedAt
    && !["onboarding", "paused", "completed", "archived"].includes(normalizeAgencyStatus(client.agencyStatus, client))
    && String(client.serviceStatus || "active") !== "paused"
    && String(client.onboardingStatus || "completed") === "completed"
  ));
}
...
async function uploadedInvoiceForReceipt({ clientCode, period }) {
  const cleanPeriod = validatePeriod(period, getInvoiceConfig().timezone);
  const upload = await findInvoiceUpload({
    clientCode,
    period: cleanPeriod,
    prefix: `INV-${periodKey(cleanPeriod)}-`,
  });
  if (!upload) return null;
  return {
    invoiceNumber: upload.invoice_number,
    servicePrice: upload.service_price,
    discount: upload.discount,
    total: upload.total,
    driveFileUrl: upload.drive_file_url || "",
  };
}

async function buildReceiptListFromClients(period, clients, config = getInvoiceConfig()) {
  const cleanPeriod = validatePeriod(period, config.timezone);
  const active = activeClients(clients);
  const uploads = await Promise.all(active.map((client) => uploadedInvoiceForReceipt({
    clientCode: client.code,
    period: cleanPeriod,
  })));

  return active.map((client, index) => {
    const sourceUpload = uploads[index];
    const overrides = sourceUpload
      ? { servicePrice: sourceUpload.servicePrice, discount: sourceUpload.discount }
      : {};
    const invoice = asReceipt(buildInvoice({ client, period: cleanPeriod, config, overrides }));
    const canResolveDriveFolder = Boolean(client.driveFolderId || (config.driveMasterFolderId && client.driveFolderName));
    const canGenerateReceipt = Boolean(sourceUpload);
    return {
      clientCode: client.code,
      clientName: client.name,
      billingName: client.billingName,
      invoiceNumber: invoice.invoiceNumber,
      sourceInvoiceNumber: sourceUpload?.invoiceNumber || invoice.invoiceNumber.replace(/^REC-/, "INV-"),
      sourceInvoiceUrl: sourceUpload?.driveFileUrl || "",
      period: invoice.period,
      periodLabel: invoice.periodLabel,
      servicePrice: canGenerateReceipt ? invoice.subtotal : 0,
      servicePriceFormatted: canGenerateReceipt ? money(invoice.subtotal, invoice.currency) : "-",
      discount: canGenerateReceipt ? invoice.discount : 0,
      discountFormatted: canGenerateReceipt ? money(invoice.discount, invoice.currency) : "-",
      amount: canGenerateReceipt ? invoice.total : 0,
      amountFormatted: canGenerateReceipt ? money(invoice.total, invoice.currency) : "-",
      fileName: invoice.fileName,
      hasDriveFolder: canResolveDriveFolder,
      canGenerateReceipt,
      receiptStatusNote: canGenerateReceipt
        ? `Ikut invoice: ${sourceUpload.invoiceNumber}`
        : "Upload invoice dahulu",
      pdfUrl: `/api/receipts/pdf?client=${encodeURIComponent(client.code)}&period=${encodeURIComponent(cleanPeriod)}`,
    };
  });
}
...
function drawLogo(doc, invoice, x, y) {
  ...
  if (invoice.business.logoBuffer) {
    doc.image(invoice.business.logoBuffer, x, y, { fit: [logoSize, logoSize], align: "center", valign: "center" });
    return;
  }

  const logoPath = invoice.business.logoPath;
  if (logoPath && fs.existsSync(logoPath)) {
    doc.image(logoPath, x, y, { fit: [logoSize, logoSize] });
    return;
  }

  doc.rect(x, y, logoSize, logoSize).fill("#f3f4f6");
  doc
    .font("Helvetica-Bold")
    .fontSize(32)
    .fillColor("#9b111e")
    .text(String(invoice.business.name || "LB").slice(0, 2).toUpperCase(), x, y + 18, {
      width: logoSize,
      align: "center",
    });
}
...
function renderInvoicePdf(invoice) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveTo(left + 82, 85).lineTo(right, 85).lineWidth(0.8).strokeColor("#b9c3be").stroke();
    ...
    const statusText = invoice.documentType === "receipt" ? "Telah Dibayar" : "Baharu";
    const statusColor = invoice.documentType === "receipt" ? "#b91c1c" : "#c59b24";
    doc
      .font("Helvetica-Bold")
      .fontSize(invoice.documentType === "receipt" ? 9 : 11)
      .fillColor(statusColor)
      .text(statusText, rightValueX + 3, 175, { width: rightValueW - 6, align: "center" });
...
async function generateReceiptPdf({ clientCode, period, overrides = {} }) {
  ...
  const client = clients.find((item) => item.code === target);
  if (!client) throw new Error(`Client ${target} tidak dijumpai dalam invoice config/registry.`);
  const sourceUpload = await uploadedInvoiceForReceipt({ clientCode: target, period: cleanPeriod });
  if (!sourceUpload) {
    throw new Error(`Upload invoice ${periodKey(cleanPeriod)} untuk ${target} dahulu sebelum generate receipt.`);
  }
  const invoice = asReceipt(buildInvoice({
    ...
    overrides: {
      ...overrides,
      servicePrice: sourceUpload.servicePrice,
      discount: sourceUpload.discount,
    },
  }));
  const buffer = await renderInvoicePdf(invoice);
  return { invoice, buffer };
...
function normalizeWhatsappPhone(rawPhone) {
  ...
  if (phone.startsWith("1")) phone = `60${phone}`;
  if (!/^\d{8,15}$/.test(phone)) {
    throw new Error("Nombor telefon client tidak sah untuk WhatsApp. Guna format seperti +60123456789.");
  }
  return phone;
}

function whatsappMessageForUpload(type, documentUrl) {
  if (type === "invoice") {
    return [
      "Invoice bulan ni dah ready. boleh terus klik link di bawah & buat bayaran.",
      "Sistem akan auto send receipt setelah bayaran dilakukan.",
      "",
      `Klik sini (Invoice): ${documentUrl}`,
    ].join("\n");
  }
  if (type === "receipt") {
    return [
      "Terima kasih untuk bayaran bulan ini. Receipt dah ready.",
      "Boleh terus klik link di bawah untuk download receipt anda.",
      "",
      `Klik sini (Receipt): ${documentUrl}`,
    ].join("\n");
  }
  if (type === "report") {
    return [
      "*Weekly Report*",
      "",
      "untuk weekly report minggu lepas dah tersedia di master files. boleh klik link di bawah untuk lihat report minggu lepas.",
      "",
      `klik sini: ${documentUrl}`,
    ].join("\n");
  }
  throw new Error("Jenis WhatsApp tidak sah.");
}
...
function mergeClients(configClients, registryClients) {
  ...
  return [...byCode.values()].filter((client) => !client.deletedAt);
}
...
function normalizeNewClientPayload(body) {
  ...
  return normalizeClient({
    ...
    email: validateAgencyEmail(body.email || body.emel, true),
    ...
    monthlyAdBudget: parseAmount(body.monthlyAdBudget || 0, "Monthly advertising budget"),
    startDate: normalizeAgencyDate(body.startDate, true),
    agencyStatus: "onboarding",
    notes: String(body.notes || "").trim(),
    driveFolderName: `${CLIENT_FOLDER_PREFIX}${brandClient}`,
    metadata: { adsReportConfig: normalizeAdsReportConfig(body) },
  });
}

function onboardingChecks(client) {
  const ads = normalizeAdsReportConfig(client.metadata?.adsReportConfig || {});
  return {
    details: Boolean(
      String(client.brandClient || "").trim()
      && String(client.contactName || "").trim()
      && String(client.phone || "").trim()
      && validateAgencyEmail(client.email, true)
      && String(client.companyName || client.billingName || "").trim()
      && Number(client.monthlyRetainer || 0) > 0
      && String(client.serviceType || "").trim()
      && normalizeAgencyDate(client.startDate, true)
    ),
    ads: Boolean(ads.platform && ads.accountId && (ads.accountName || ads.accountId) && ads.currency && ads.resultMetric),
    drive: Boolean(client.driveFolderId && client.weeklyReportFolderId && client.invoiceReceiptFolderId),
    telegram: Boolean(client.metadata?.telegramReportConfig),
  };
}

function nextOnboardingState(client, step, status = "complete", error = "") {
  const now = new Date().toISOString();
  const current = client.onboardingState && typeof client.onboardingState === "object" ? client.onboardingState : {};
  return {
    ...current,
    version: 1,
    steps: {
      ...(current.steps || {}),
      [step]: { status, error: String(error || ""), updatedAt: now },
    },
    updatedAt: now,
  };
}

function assertOnboardingDetails(client) {
  const checks = onboardingChecks(client);
  if (!String(client.brandClient || "").trim()) throw new Error("Brand client wajib diisi.");
  if (!String(client.contactName || "").trim()) throw new Error("Nama PIC wajib diisi.");
  if (!String(client.phone || "").trim()) throw new Error("Nombor telefon WhatsApp client wajib diisi.");
  validateAgencyEmail(client.email, true);
  if (!String(client.companyName || client.billingName || "").trim()) throw new Error("Nama syarikat wajib diisi.");
  if (Number(client.monthlyRetainer || 0) <= 0) throw new Error("Harga service mesti lebih daripada 0.");
  if (!String(client.serviceType || "").trim()) throw new Error("Service type wajib diisi.");
  normalizeAgencyDate(client.startDate, true);
  return checks;
}

async function findOnboardingClient(code) {
  const target = cleanClientCode(code);
  const { clients } = await getMergedClientsWithStatus();
  const client = clients.find((item) => item.code === target && !item.deletedAt);
  if (!client) throw new Error(`Client ${target} tidak dijumpai.`);
  return client;
}

async function startClientOnboarding(body) {
  if (!isSupabaseConfigured()) throw new Error("Supabase wajib disambungkan untuk onboarding client.");
  const candidate = normalizeNewClientPayload(body);
  const { clients } = await getMergedClientsWithStatus();
  const targetBrand = normalizeFolderName(candidate.brandClient);
  if (clients.some((client) => !client.deletedAt && (
    client.code === candidate.code
    || normalizeFolderName(client.brandClient || client.name) === targetBrand
  ))) {
    throw new Error(`Client ${candidate.brandClient} sudah wujud.`);
  }
  assertOnboardingDetails(candidate);
  const now = new Date().toISOString();
  const draft = normalizeClient({
    ...
    onboardingStep: "ads",
    onboardingState: nextOnboardingState(candidate, "details"),
    onboardingCompletedAt: "",
    agencyStatus: "onboarding",
    createdAt: now,
    updatedAt: now,
  });
  return normalizeClient(await upsertSupabaseClient(draft));
}

async function saveClientOnboardingDetails(body) {
  const existing = await findOnboardingClient(body.clientCode || body.code);
  if (existing.onboardingStatus !== "in_progress") throw new Error("Onboarding client ini sudah selesai.");
  const patch = clientEditPatch(body, existing);
  const candidate = normalizeClient({ ...existing, ...patch });
  assertOnboardingDetails(candidate);
  candidate.onboardingStep = "ads";
  candidate.onboardingState = nextOnboardingState(candidate, "details");
  return normalizeClient(await upsertSupabaseClient(candidate));
}

async function saveClientOnboardingAds(body) {
  const existing = await findOnboardingClient(body.clientCode || body.code);
  if (existing.onboardingStatus !== "in_progress") throw new Error("Onboarding client ini sudah selesai.");
  assertOnboardingDetails(existing);
  const adsReportConfig = normalizeAdsReportConfig({
    ...(existing.metadata?.adsReportConfig || {}),
    ...body,
  });
  if (!adsReportConfig.accountId) throw new Error("Pilih akaun Ads untuk client ini.");
  const candidate = normalizeClient({
    ...existing,
    metadata: { ...existing.metadata, adsReportConfig },
    onboardingStep: "drive",
  });
  candidate.onboardingState = nextOnboardingState(candidate, "ads");
  return normalizeClient(await upsertSupabaseClient(candidate));
}

async function verifyOrEnsureOnboardingFolder(drive, folderId, parentFolderId, folderName) {
  if (folderId) {
    try {
      const response = await drive.files.get({
        fileId: folderId,
      ...
      if (response.data?.id && !response.data.trashed) return { id: response.data.id, name: response.data.name || folderName };
    } catch (error) {
      if (!/not found|404/i.test(error?.message || "") && Number(error?.code || error?.response?.status) !== 404) throw error;
    }
  }
  return ensureDriveFolder(drive, parentFolderId, folderName);
}

async function provisionClientOnboardingDrive(body) {
  const config = getInvoiceConfig();
  const masterFolderId = String(config.driveMasterFolderId || "").trim();
  if (!masterFolderId) throw new Error("INVOICE_DRIVE_MASTER_FOLDER_ID belum diset.");
  const existing = await findOnboardingClient(body.clientCode || body.code);
  ...
  const drive = getDriveClient();
  const folderName = existing.driveFolderName || `${CLIENT_FOLDER_PREFIX}${existing.brandClient}`;
  const clientFolder = await verifyOrEnsureOnboardingFolder(drive, existing.driveFolderId, masterFolderId, folderName);
  await ensureAnyoneWriterPermission(drive, clientFolder.id);
  const weeklyFolder = await verifyOrEnsureOnboardingFolder(drive, existing.weeklyReportFolderId, clientFolder.id, WEEKLY_REPORT_FOLDER);
  const invoiceFolder = await verifyOrEnsureOnboardingFolder(drive, existing.invoiceReceiptFolderId, clientFolder.id, INVOICE_RECEIPT_FOLDER);
  const candidate = normalizeClient({
    ...existing,
    driveFolderId: clientFolder.id,
    driveFolderName: clientFolder.name || folderName,
    weeklyReportFolderId: weeklyFolder.id,
    invoiceReceiptFolderId: invoiceFolder.id,
    onboardingStep: onboardingChecks(existing).ads ? "telegram" : "ads",
  });
  candidate.onboardingState = nextOnboardingState(candidate, "drive");
  return normalizeClient(await upsertSupabaseClient(candidate));
}

async function updateClientOnboardingTelegram(body) {
  const existing = await findOnboardingClient(body.clientCode || body.code);
  if (existing.onboardingStatus !== "in_progress") throw new Error("Onboarding client ini sudah selesai.");
  const status = body.status === "complete" ? "complete" : "skipped";
  const candidate = normalizeClient({ ...existing, onboardingStep: "review" });
  candidate.onboardingState = nextOnboardingState(candidate, "telegram", status);
  return normalizeClient(await upsertSupabaseClient(candidate));
}

async function markClientOnboardingError(body, action, error) {
  const existing = await findOnboardingClient(body.clientCode || body.code);
  if (existing.onboardingStatus !== "in_progress") return existing;
  const step = action === "drive" && !onboardingChecks(existing).ads
    ? existing.onboardingStep || "ads"
    : (["details", "ads", "drive", "telegram", "review"].includes(action) ? action : existing.onboardingStep || "details");
  const candidate = normalizeClient({ ...existing, onboardingStep: step });
  candidate.onboardingState = nextOnboardingState(candidate, step, "error", error?.message || String(error));
  return normalizeClient(await upsertSupabaseClient(candidate));
}

async function activateClientOnboarding(body) {
  const existing = await findOnboardingClient(body.clientCode || body.code);
  if (existing.onboardingStatus !== "in_progress") return existing;
  const checks = onboardingChecks(existing);
  if (!checks.details || !checks.ads || !checks.drive) {
    throw new Error("Billing, akaun Ads dan folder Drive mesti siap sebelum client diaktifkan.");
  }
  const now = new Date().toISOString();
  const candidate = normalizeClient({
    ...existing,
    ...
    onboardingState: { ...nextOnboardingState(existing, "review"), completedAt: now },
    agencyStatus: "active",
...
async function discardClientOnboarding(body) {
  const existing = await findOnboardingClient(body.clientCode || body.code);
  if (existing.onboardingStatus !== "in_progress") throw new Error("Hanya onboarding yang belum siap boleh dibuang.");
  if (existing.driveFolderId) {
    return deleteClientPermanently({ clientCode: existing.code, confirmName: existing.brandClient });
  }
  await deleteSupabaseOnboardingDraft(existing.code);
  return { client: existing, discarded: true };
}
...
function clientEditPatch(body, existingClient) {
  ...
  if (Object.prototype.hasOwnProperty.call(body, "email") || Object.prototype.hasOwnProperty.call(body, "emel")) {
    patch.email = validateAgencyEmail(body.email || body.emel, true);
  }
  ...
  if (Object.prototype.hasOwnProperty.call(body, "serviceType")) {
    ...
    if (!patch.serviceType) throw new Error("Service type wajib diisi.");
  }
  if (Object.prototype.hasOwnProperty.call(body, "monthlyAdBudget")) {
    patch.monthlyAdBudget = parseAmount(body.monthlyAdBudget || 0, "Monthly advertising budget");
  }
  if (Object.prototype.hasOwnProperty.call(body, "startDate")) {
    patch.startDate = normalizeAgencyDate(body.startDate, true);
  }
  if (Object.prototype.hasOwnProperty.call(body, "notes")) {
    patch.notes = String(body.notes || "").trim();
  }
  if (Object.prototype.hasOwnProperty.call(body, "agencyStatus")) {
    const agencyStatus = normalizeAgencyStatus(body.agencyStatus, existingClient);
    const now = new Date().toISOString();
    patch.agencyStatus = agencyStatus;
    patch.archivedAt = agencyStatus === "archived" ? (existingClient.archivedAt || now) : "";
    patch.onboardingStatus = agencyStatus === "onboarding" ? "in_progress" : "completed";
    patch.serviceStatus = agencyStatus === "active" ? "active" : "paused";
    if (agencyStatus === "paused" || agencyStatus === "completed" || agencyStatus === "archived") {
      patch.serviceStoppedAt = existingClient.serviceStoppedAt || now;
    }
    if (agencyStatus === "active") patch.serviceRecoveredAt = now;
  }

  const adsFields = ["platform", "accountId", "accountName", "currency", "resultMetric", "prospectingKeywords", "retargetingKeywords"];
  if (adsFields.some((field) => Object.prototype.hasOwnProperty.call(body, field))) {
    patch.metadata = {
      ...(existingClient.metadata || {}),
      adsReportConfig: normalizeAdsReportConfig({
        ...(existingClient.metadata?.adsReportConfig || {}),
        ...body,
      }),
    };
  }

  if (Object.prototype.hasOwnProperty.call(body, "telegramReportConfig")) {
    const nextTelegram = body.telegramReportConfig && typeof body.telegramReportConfig === "object"
      ? body.telegramReportConfig
      : {};
    patch.metadata = {
      ...(patch.metadata || existingClient.metadata || {}),
      telegramReportConfig: {
        ...(existingClient.metadata?.telegramReportConfig || {}),
        ...nextTelegram,
      },
    };
  }
...
async function setClientServiceStatus(body) {
  ...
  registry.clients[registryIndex] = {
    ...
    agencyStatus: status === "paused" ? "paused" : "active",
    archivedAt: "",
...
async function createClientWithDriveFolders(body) {
  ...
  const allExisting = mergedClients;
...
async function buildClientWhatsappReminder(body) {
  const clientCode = cleanClientCode(body.clientCode || body.code);
  if (!clientCode) throw new Error("Client code wajib ada.");
  ...
  const client = clients.find((item) => item.code === clientCode);
  if (!client) throw new Error(`Client ${clientCode} tidak dijumpai.`);
  if (!client.phone) throw new Error(`Tambah nombor telefon untuk ${client.brandClient || client.name} dahulu.`);
  ...
  if (type === "custom") {
    ...
    if (!message) throw new Error("Custom message wajib diisi.");
  } else if (type === "invoice" || type === "receipt") {
    const upload = await findInvoiceUpload({
      clientCode,
      period,
      prefix: type === "invoice" ? `INV-${periodKey(period)}-` : `REC-${periodKey(period)}-`,
    });
    documentUrl = upload?.drive_file_url || "";
    if (!documentUrl) {
      throw new Error(type === "invoice"
        ? "Invoice bulan ini belum di-upload ke Google Drive. Upload invoice dahulu."
        : "Receipt bulan ini belum di-upload ke Google Drive. Upload receipt dahulu.");
    }
    message = whatsappMessageForUpload(type, documentUrl);
  } else if (type === "report") {
    const drive = getDriveClient();
    const latestReport = await getLatestWeeklyReportForClient(drive, client, config);
    documentUrl = latestReport.webViewLink || "";
    message = whatsappMessageForUpload(type, documentUrl);
  } else {
    throw new Error("Pilih WhatsApp Invoice, Receipt, Report, atau Custom Message.");
  }
...
async function deleteClientPermanently(body) {
  const clientCode = cleanClientCode(body.clientCode || body.code);
  if (!clientCode) throw new Error("Client code wajib ada.");
  const confirmName = String(body.confirmName || "").trim();
  if (!confirmName) throw new Error("Taip nama client untuk confirm delete.");
  ...
  if (!validNames.includes(confirmName)) {
    throw new Error(`Nama confirm tidak sama. Taip tepat: ${client.brandClient || client.name}.`);
  }

  if (isSupabaseConfigured()) {
    await assertSupabaseClientDeleteReady();
  }
  ...
  const folderName = folder.data.name || client.driveFolderName || client.name;
  ...
  if (isSupabaseConfigured()) {
    const saved = await tombstoneSupabaseClient({
      client,
      driveFolderId: folderId,
  ...
  const registry = await readClientRegistry(drive, masterFolderId);
  const now = new Date().toISOString();
  const registryIndex = (registry.clients || []).findIndex((item) => {
    const registryCode = cleanClientCode(item.code || clientCodeFromBrand(item.brandClient || item.name));
    return registryCode === clientCode;
  });
  const tombstone = normalizeClient({
    ...client,
    source: "drive-registry",
    deletedAt: now,
    deletedDriveFolderId: folderId,
    deletedDriveFolderName: folderName,
    driveFolderId: "",
    weeklyReportFolderId: "",
    invoiceReceiptFolderId: "",
    serviceStatus: "paused",
    serviceStoppedAt: client.serviceStoppedAt || now,
  });

  if (registryIndex >= 0) {
    registry.clients[registryIndex] = tombstone;
  } else {
    registry.clients.push(tombstone);
  }
  const registryFile = await writeClientRegistry(drive, masterFolderId, registry);
  return {
    client: tombstone,
...
async function findNamedSubfolder(drive, parentFolderId, folderName) {
  const response = await drive.files.list({
    q: [
      `'${driveQueryValue(parentFolderId)}' in parents`,
      "mimeType = 'application/vnd.google-apps.folder'",
...
async function findLatestWeeklyReportFile(drive, folderId) {
  const response = await drive.files.list({
    q: [
      `'${driveQueryValue(folderId)}' in parents`,
...
async function getLatestWeeklyReportForClient(drive, client, config) {
  let weeklyReportFolderId = client.weeklyReportFolderId || "";

  if (!weeklyReportFolderId) {
    const clientFolderId = await resolveClientDriveFolder(drive, client, config);
    const weeklyFolder = await findNamedSubfolder(drive, clientFolderId, WEEKLY_REPORT_FOLDER);
    weeklyReportFolderId = weeklyFolder?.id || "";
  }

  if (!weeklyReportFolderId) {
    throw new Error("Weekly report belum dijumpai dalam folder Weekly Report client. Upload report dahulu.");
  }

  const latestReport = await findLatestWeeklyReportFile(drive, weeklyReportFolderId);
  if (!latestReport?.id) {
    throw new Error("Weekly report belum dijumpai dalam folder Weekly Report client. Upload report dahulu.");
  }
  ...
  return {
    ...latestReport,
    weeklyReportFolderId,
    webViewLink: latestReport.webViewLink || `https://drive.google.com/file/d/${encodeURIComponent(latestReport.id)}/view?usp=sharing`,
  };
}
...
async function uploadInvoices({ period, drafts = [] }) {
  ...
  const cleanPeriod = validatePeriod(period, config.timezone);
  const drive = getDriveClient();
  const selectedDrafts = (Array.isArray(drafts) ? drafts : []).filter((draft) => draft.selected || draft.upload);
  if (!selectedDrafts.length) throw new Error("Pilih sekurang-kurangnya satu invoice untuk upload.");
...
async function uploadReceipts({ period, drafts = [] }) {
  ...
  for (const client of activeClients(clients).filter((item) => selectedCodes.has(item.code))) {
    const sourceUpload = await uploadedInvoiceForReceipt({ clientCode: client.code, period: cleanPeriod });
    if (!sourceUpload) {
      throw new Error(`Upload invoice ${periodKey(cleanPeriod)} untuk ${client.name} dahulu sebelum upload resit.`);
    }
    const invoice = asReceipt(buildInvoice({
      client,
      period: cleanPeriod,
...
function parseReportDate(value, fieldName) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(clean)) throw new Error(`${fieldName} mesti format YYYY-MM-DD.`);
...
function reportDateParts(iso) {
  const [year, month, day] = iso.split("-").map(Number);
  return { year, month, day };
}

function reportMonthName(month, locale = "en-MY", style = "long") {
  return new Intl.DateTimeFormat(locale, { month: style }).format(new Date(Date.UTC(2026, month - 1, 1)));
}

function reportDateRangeLabel(startDate, endDate) {
  const start = reportDateParts(startDate);
  const end = reportDateParts(endDate);
  if (start.year === end.year && start.month === end.month) {
    return `${reportMonthName(start.month).toUpperCase()} ${start.day}-${end.day}, ${end.year}`;
  }
  return `${reportMonthName(start.month).toUpperCase()} ${start.day} - ${reportMonthName(end.month).toUpperCase()} ${end.day}, ${end.year}`;
}

function reportFileDateRange(startDate, endDate) {
  const start = reportDateParts(startDate);
  const end = reportDateParts(endDate);
  const startMonth = reportMonthName(start.month, "en-MY", "long").replace(/\s+/g, "");
  const endMonth = reportMonthName(end.month, "en-MY", "long").replace(/\s+/g, "");
  const startDay = String(start.day).padStart(2, "0");
  const endDay = String(end.day).padStart(2, "0");
  if (start.year === end.year && start.month === end.month) return `${startMonth}${startDay}-${endDay}_${end.year}`;
  return `${startMonth}${startDay}-${endMonth}${endDay}_${end.year}`;
}
...
function reportNumber(value, fieldName, options = {}) {
  const clean = String(value ?? "").replace(/,/g, "").trim();
  const number = clean === "" ? 0 : Number(clean);
  if (!Number.isFinite(number) || number < 0) throw new Error(`${fieldName} mesti nombor 0 atau lebih.`);
  return options.integer ? Math.round(number) : Math.round(number * 100) / 100;
}

function reportMoney(value, currency = "MYR") {
  if (value == null || value === "") return "N/A";
  const number = Number(value || 0);
  try {
    return new Intl.NumberFormat("en-MY", {
      style: "currency",
      currency: String(currency || "MYR").toUpperCase(),
      minimumFractionDigits: Number.isInteger(number) ? 0 : 2,
      maximumFractionDigits: 2,
    }).format(number);
  } catch {
    return `${String(currency || "MYR").toUpperCase()} ${number.toLocaleString("en-MY", { maximumFractionDigits: 2 })}`;
  }
}

function cleanReportFileName(value) {
  const clean = safeFilePart(value).replace(/\.pdf$/i, "").trim();
  if (!clean) throw new Error("Nama fail report wajib ada.");
  return `${clean}.pdf`;
}

function defaultReportFileName(client, startDate, endDate) {
  return cleanReportFileName(`${client.name || client.brandClient || client.code} Weekly Report ${reportFileDateRange(startDate, endDate)}`);
}

function normalizeReportInput(body = {}, client) {
  const startDate = parseReportDate(body.startDate || body.reportStartDate, "Tarikh mula report");
  const endDate = parseReportDate(body.endDate || body.reportEndDate, "Tarikh akhir report");
  if (new Date(`${endDate}T00:00:00Z`) < new Date(`${startDate}T00:00:00Z`)) {
    throw new Error("Tarikh akhir report tidak boleh sebelum tarikh mula.");
  }

  const adSpend = reportNumber(body.adSpend, "Ad spend");
  const leadsGenerated = reportNumber(body.leadsGenerated, "Results", { integer: true });
  const costPerLead = Object.prototype.hasOwnProperty.call(body, "costPerLead") && body.costPerLead != null && String(body.costPerLead).trim() !== ""
    ? reportNumber(body.costPerLead, "Cost per result")
    : (leadsGenerated ? Math.round((adSpend / leadsGenerated) * 100) / 100 : null);
  const requestedPhase = String(body.phase || "SETUP PHASE").trim().toUpperCase();
  const allowedPhases = new Set(["SETUP PHASE", "TESTING PHASE", "OPTIMIZE PHASE", "SCALING PHASE"]);

  return {
    client,
    startDate,
    endDate,
    dateRangeLabel: reportDateRangeLabel(startDate, endDate),
    phase: allowedPhases.has(requestedPhase) ? requestedPhase : "SETUP PHASE",
    reportTitle: String(body.reportTitle || "META ADS PERFORMANCE BRIEF").trim().toUpperCase(),
    adSpend,
    leadsGenerated,
    costPerLead,
    currency: String(body.currency || "MYR").trim().toUpperCase() || "MYR",
    resultLabel: String(body.resultLabel || "Results").trim() || "Results",
    whatWeProved: splitReportLines(body.whatWeProved),
    winningCreative: String(body.winningCreative || "N/A").trim() || "N/A",
    bestPerformance: String(body.bestPerformance || "").trim(),
    retargetingWinningCreative: String(body.retargetingWinningCreative || "N/A").trim() || "N/A",
    retargetingBestPerformance: String(body.retargetingBestPerformance || "").trim(),
    bestAudience: String(body.bestAudience || "N/A").trim() || "N/A",
    leadLeaks: splitReportLines(body.leadLeaks),
    next7Days: splitReportLines(body.next7Days),
    recommendation: String(body.recommendation || "").trim(),
    recommendationHeadline: String(body.recommendationHeadline || "DATA-DRIVEN RECOMMENDATION").trim().toUpperCase(),
    preparedBy: String(body.preparedBy || "Abdussomad Ruddin | Growth Partner").trim(),
    fileName: cleanReportFileName(body.fileName || defaultReportFileName(client, startDate, endDate)),
  };
}

function writeReportWrapped(doc, text, x, y, width, options = {}) {
  ...
  doc.font(options.bold ? "Helvetica-Bold" : "Helvetica").fontSize(options.size || 8.5);
  let rendered = value;
  let height = doc.heightOfString(rendered, textOptions);
  if (options.height && height > options.height) {
    ...
    while (low < high) {
      ...
      if (doc.heightOfString(candidate, textOptions) <= options.height) low = middle;
      else high = middle - 1;
    }
    rendered = low < value.length ? `${value.slice(0, low).trimEnd()}...` : value;
    height = doc.heightOfString(rendered, textOptions);
  }
  doc.fillColor(options.color || "#14213d").text(rendered, x, y, textOptions);
  return Math.min(height, options.height || height);
}

function reportSection(doc, title, x, y, width, height, options = {}) {
  doc.roundedRect(x, y, width, height, 10).fill(options.fill || "#f8fbff");
  doc
    .font("Helvetica-Bold")
    .fontSize(options.titleSize || 8.5)
    .fillColor(options.titleColor || "#ff2442")
...
function renderReportPdf(report) {
  return new Promise((resolve, reject) => {
    ...
    doc.rect(0, 0, pageW, doc.page.height).fill("#fff7df");
    doc.rect(0, 0, pageW, 96).fill(red);
    doc
      .font("Helvetica-Bold")
      .fontSize(28)
      .fillColor("#ffffff")
      .text("WEEKLY REPORT", margin, 25, { width: contentW, align: "center" });
    doc
      .fontSize(11)
      .fillColor(yellow)
      .text(report.phase, margin, 58, { width: contentW, align: "center" });
    doc
      .fontSize(10)
      .fillColor("#ffffff")
      .text(report.reportTitle, margin, 75, { width: contentW, align: "center" });

    doc
      .font("Helvetica-Bold")
      .fontSize(12)
      .fillColor(ink)
      .text(`${report.client.name || report.client.brandClient} | ${report.dateRangeLabel}`, margin, 112, { width: contentW, align: "center" });

    const metricY = 143;
    const gap = 12;
    const metricW = (contentW - gap * 2) / 3;
    const metrics = [
      ["AD SPEND", reportMoney(report.adSpend, report.currency), blue],
      [report.resultLabel.toUpperCase(), String(report.leadsGenerated), green],
      [
        report.resultLabel === "Leads"
          ? "COST PER LEAD (CPL)"
          : report.resultLabel === "Clicks"
            ? "COST PER CLICK (CPC)"
            : "COST PER RESULT",
        reportMoney(report.costPerLead, report.currency),
        red,
      ],
    ];
    metrics.forEach(([label, value, color], index) => {
      const x = margin + index * (metricW + gap);
      doc.roundedRect(x, metricY, metricW, 75, 12).fill("#ffffff");
      doc.roundedRect(x, metricY, metricW, 8, 8).fill(color);
      doc.font("Helvetica-Bold").fontSize(8).fillColor(muted).text(label, x + 10, metricY + 20, { width: metricW - 20, align: "center" });
      doc.fontSize(23).fillColor(ink).text(value, x + 10, metricY + 39, { width: metricW - 20, align: "center" });
    });
    ...
    reportSection(doc, "WHAT WE PROVED", leftX, 238, colW, 127);
    const proofLines = report.whatWeProved.length ? report.whatWeProved : ["Report details belum diisi."];
    let proofY = 264;
    const proofBottom = 355;
    for (const line of proofLines.slice(0, 6)) {
      const remaining = proofBottom - proofY;
      if (remaining < 9) break;
      doc.font("Helvetica").fontSize(7.2);
      const naturalHeight = doc.heightOfString(line, { width: colW - 37, lineGap: 1 });
      const itemHeight = Math.min(naturalHeight, remaining);
      doc.circle(leftX + 16, proofY + 3.5, 2.3).fill(green);
      writeReportWrapped(doc, line, leftX + 25, proofY, colW - 37, { size: 7.2, lineGap: 1, height: itemHeight });
      proofY += itemHeight + 4;
    }

    reportSection(doc, "BEST PERFORMERS", rightX, 238, colW, 127);
    writeReportWrapped(doc, `Prospecting Ad: ${report.winningCreative}`, rightX + 12, 263, colW - 24, { bold: true, size: 7.5, height: 15 });
    writeReportWrapped(doc, report.bestPerformance || "N/A", rightX + 12, 281, colW - 24, { size: 7.1, lineGap: 1, height: 17 });
    doc.moveTo(rightX + 12, 303).lineTo(rightX + colW - 12, 303).strokeColor("#dbeafe").lineWidth(1).stroke();
    writeReportWrapped(doc, `Retargeting Ad: ${report.retargetingWinningCreative}`, rightX + 12, 309, colW - 24, { bold: true, size: 7.5, height: 15 });
    writeReportWrapped(doc, report.retargetingBestPerformance || "N/A", rightX + 12, 327, colW - 24, { size: 7.1, lineGap: 1, height: 17 });

    reportSection(doc, "PERFORMANCE LEAKS", leftX, 382, colW, 102);
    const leaks = report.leadLeaks.length ? report.leadLeaks : ["Tiada performance leak kritikal direkodkan untuk tempoh ini."];
    let leakY = 410;
    leaks.slice(0, 4).forEach((line) => {
      const remaining = 473 - leakY;
      if (remaining < 9) return;
      doc.circle(leftX + 16, leakY + 4, 2.1).fill(red);
      const height = writeReportWrapped(doc, line, leftX + 24, leakY, colW - 36, { size: 7.5, lineGap: 1, height: remaining });
      leakY += height + 6;
    });

    doc.roundedRect(rightX, 382, colW, 102, 10).fill("#ffffff");
    doc.font("Helvetica-Bold").fontSize(8.5).fillColor(red).text("CURRENT POSITION", rightX + 12, 398, { width: colW / 2 - 20, align: "center" });
    doc.font("Helvetica-Bold").fontSize(8.5).fillColor(red).text("CMO DIRECTION", rightX + colW / 2 + 4, 398, { width: colW / 2 - 20, align: "center" });
    doc.moveTo(rightX + colW / 2, 392).lineTo(rightX + colW / 2, 474).strokeColor("#e8f0ff").lineWidth(2).stroke();
    doc.font("Helvetica-Bold").fontSize(19).fillColor(ink).text(`${report.leadsGenerated} ${report.resultLabel}`, rightX + 12, 424, { width: colW / 2 - 20, align: "center" });
    const currentCostLabel = report.resultLabel === "Leads"
      ? `CPL ${reportMoney(report.costPerLead, report.currency)}`
      : report.resultLabel === "Clicks"
        ? `CPC ${reportMoney(report.costPerLead, report.currency)}`
        : `${reportMoney(report.costPerLead, report.currency)} / result`;
    doc.fontSize(12).fillColor(muted).text(currentCostLabel, rightX + 12, 449, { width: colW / 2 - 20, align: "center" });
    writeReportWrapped(doc, report.recommendationHeadline || "DATA-DRIVEN RECOMMENDATION", rightX + colW / 2 + 8, 425, colW / 2 - 16, { bold: true, size: 10.5, lineGap: 1.5, height: 43, align: "center" });

    reportSection(doc, "NEXT 7 DAYS", margin, 503, contentW, 125);
    const nextItems = report.next7Days.length ? report.next7Days : ["Monitor campaign performance.", "Review cost and result quality.", "Prepare next optimization action."];
    let nextY = 531;
    nextItems.slice(0, 6).forEach((line, index) => {
      if (nextY > 616) return;
      const rowY = nextY;
      doc.roundedRect(margin + 14, rowY - 2, 16, 13, 4).fill(yellow);
      doc.font("Helvetica-Bold").fontSize(7.5).fillColor(ink).text(String(index + 1), margin + 14, rowY, { width: 16, align: "center" });
      const height = writeReportWrapped(doc, line, margin + 38, rowY - 1, contentW - 54, { size: 8, lineGap: 1, height: 624 - rowY });
      nextY += Math.max(15, height + 5);
    });

    reportSection(doc, "EXECUTIVE RECOMMENDATION", margin, 648, contentW, 100, { fill: "#ffffff" });
    const recommendationItems = splitReportLines(report.recommendation || "Recommendation belum diisi.");
    let recommendationY = 677;
    recommendationItems.slice(0, 4).forEach((line) => {
      if (recommendationY > 735) return;
      doc.circle(margin + 17, recommendationY + 4, 2.2).fill(red);
      const height = writeReportWrapped(doc, line, margin + 26, recommendationY, contentW - 40, { size: 8.2, lineGap: 1, height: 742 - recommendationY });
      recommendationY += Math.max(13, height + 4);
    });
    ...
    doc
      ...
      .fontSize(8.5)
      .fillColor(muted)
      .text(`Prepared By: ${report.preparedBy}`, margin, 775, { width: contentW, align: "center" });

    doc.end();
  });
}

async function buildReportPdf(body = {}) {
  const clientCode = cleanClientCode(body.clientCode || body.code || body.client);
  const { clients } = await getMergedClientsWithStatus();
  const client = activeClients(clients).find((item) => item.code === clientCode);
  if (!client) throw new Error(`Client ${clientCode} tidak dijumpai atau tidak aktif.`);
  const report = normalizeReportInput(body, client);
  const buffer = await renderReportPdf(report);
  return { report, buffer };
}

async function uploadReportToDrive(body = {}) {
  const { config, clients } = await getMergedClientsWithStatus();
  const clientCode = cleanClientCode(body.clientCode || body.code || body.client);
  const client = activeClients(clients).find((item) => item.code === clientCode);
  if (!client) throw new Error(`Client ${clientCode} tidak dijumpai atau tidak aktif.`);
  ...
  const drive = getDriveClient();
  const clientFolderId = await resolveClientDriveFolder(drive, client, config);
  const weeklyReportFolderId = client.weeklyReportFolderId || await ensureNamedSubfolder(drive, clientFolderId, WEEKLY_REPORT_FOLDER);
  const existing = await findDriveFileByName(drive, weeklyReportFolderId, report.fileName);
  const media = {
    mimeType: "application/pdf",
    body: Readable.from(buffer),
  };
  ...
  if (existing?.id) {
    ...
    upload = { ...updated.data, replaced: true };
  } else {
    const created = await drive.files.create({
      requestBody: {
        name: report.fileName,
        mimeType: "application/pdf",
        parents: [weeklyReportFolderId],
      },
      media,
      fields: "id, name, webViewLink",
      supportsAllDrives: true,
    });
    upload = { ...created.data, replaced: false };
  }

  await recordActivity({
    type: "report_uploaded",
    title: `${report.client.name || report.client.brandClient} weekly report uploaded`,
    description: `${report.dateRangeLabel} - ${upload.replaced ? "replaced" : "uploaded"}`,
    entityType: "report",
    entityId: upload.id || report.fileName,
    metadata: {
      clientCode: report.client.code,
      fileName: report.fileName,
      fileId: upload.id || "",
      webViewLink: upload.webViewLink || "",
      replaced: Boolean(upload.replaced),
      weeklyReportFolderId,
    },
  });

  let reportWhatsappUrl = "";
  let reportWhatsappMessage = "";
  let reportWhatsappError = "";
  try {
    const phone = normalizeWhatsappPhone(report.client.phone);
    reportWhatsappMessage = weeklyReportWhatsappMessage({
      contactName: report.client.contactName,
      clientName: report.client.name || report.client.brandClient,
      dateRangeLabel: report.dateRangeLabel,
      documentUrl: upload.webViewLink,
    });
    reportWhatsappUrl = whatsappUrl(phone, reportWhatsappMessage);
  } catch (error) {
    reportWhatsappError = error?.message || String(error);
  }
  ...
  return {
    ...
    fileId: upload.id,
    webViewLink: upload.webViewLink,
    whatsappUrl: reportWhatsappUrl,
    whatsappMessage: reportWhatsappMessage,
    whatsappError: reportWhatsappError,
    replaced: Boolean(upload.replaced),
    weeklyReportFolderId,
  };
}
