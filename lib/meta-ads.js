...
async function listMetaAdAccountsWithClient(client) {
  ...
  const tool = candidates.find((item) => ["list_ad_accounts", "get_ad_accounts", "meta_ads_list_ad_accounts"].includes(item.name))
    || candidates[0];
...
async function listMetaAdAccounts() {
  try {
    return await withMetaClient(listMetaAdAccountsWithClient);
  } catch (error) {
    const { listPersonalAdsAccounts } = require("./supabase-db");
    const stored = await listPersonalAdsAccounts();
    const fallback = stored.map((account) => ({
      id: cleanText(account.account_id).replace(/^act_/, ""),
      name: cleanText(account.account_name || account.account_id),
      currency: cleanText(account.currency || "MYR").toUpperCase(),
      platform: "meta",
    })).filter((account) => account.id);
    if (fallback.length) return fallback.sort((a, b) => a.name.localeCompare(b.name));
    throw error;
  }
}
