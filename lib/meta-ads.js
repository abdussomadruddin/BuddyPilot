...
async function listMetaAdAccounts() {
  ...
  try {
    ...
    return discovered.length ? discovered : storedAccounts();
  } catch (error) {
...
async function fetchMetaRange(configInput, startDate, endDate, { full = true } = {}) {
  ...
  const definitions = [
    ...
    ...(full ? [
      ["adset", `id,name,campaign_id,campaign_name,${base}`],
      ["ad", `id,name,campaign_id,campaign_name,adset_id,adset_name,${base},quality_ranking,engagement_rate_ranking,conversion_rate_ranking`],
    ] : []),
  ];
...
async function fetchMetaCustomReport(configInput, startDate, endDate) {
  ...
  const timeRange = JSON.stringify({ since: startDate, until: endDate });
  const definitions = [
    ["account", "id,name,spend,impressions,clicks,inline_link_clicks,reach,frequency,cpm,cpc,ctr,actions,purchase_roas"],
    ["campaign", "id,name,spend,impressions,reach,frequency,clicks,inline_link_clicks,cpm,cpc,ctr,actions,action_values,purchase_roas"],
    ["adset", "id,name,campaign_id,campaign_name,spend,impressions,clicks,inline_link_clicks,cpm,cpc,ctr,actions,action_values,purchase_roas"],
    ["ad", "id,name,campaign_id,campaign_name,adset_id,adset_name,spend,impressions,clicks,inline_link_clicks,cpm,cpc,ctr,actions,action_values,purchase_roas,quality_ranking,engagement_rate_ranking,conversion_rate_ranking"],
  ];
  const results = await Promise.all(definitions.map(([level, fields]) => metaMcpCall("meta_api_get", {
