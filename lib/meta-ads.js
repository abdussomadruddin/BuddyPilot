...
const META_GRAPH_API_URL = cleanText(process.env.META_GRAPH_API_URL || "https://graph.facebook.com/v25.0").replace(/\/+$/, "");
...
function isMcpAccountUnavailable(error) {
  return /not enabled for the ads mcp|gradually rolled out across ad accounts/i.test(cleanText(error?.message || error));
}

function graphAccessToken(state) {
  const tokens = state?.tokens || {};
  return cleanText(tokens.access_token || tokens.accessToken || tokens.token);
}

async function metaGraphInsights(accountId, params, state) {
  const token = graphAccessToken(state);
  if (!token) throw new Error("Authorization Meta tidak mempunyai access token untuk fallback Graph API. Reauthorize Meta Ads.");
  const account = cleanText(accountId).replace(/^act_/, "");
  const url = new URL(`${META_GRAPH_API_URL}/act_${account}/insights`);
  Object.entries(params || {}).forEach(([key, value]) => {
    if (value !== undefined && value !== null && value !== "") url.searchParams.set(key, typeof value === "string" ? value : JSON.stringify(value));
  });
  url.searchParams.set("access_token", token);
  const response = await fetch(url, { headers: { accept: "application/json" } });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok || payload?.error) throw new Error(cleanText(payload?.error?.message || `Meta Graph API request gagal (${response.status}).`));
  return Array.isArray(payload?.data) ? payload.data : [];
}

async function fetchGraphLevels(config, startDate, endDate, definitions) {
  const { state } = await loadStoredConnection();
  const timeRange = { since: startDate, until: endDate };
  const results = await Promise.all(definitions.map(async ([level, fields]) => [
    level,
    await metaGraphInsights(config.accountId, { fields, level, time_range: timeRange, limit: 500 }, state),
  ]));
  return Object.fromEntries(results);
}

function fallbackWarning() {
  return "Meta Ads MCP belum diaktifkan untuk account ini. Data dibaca melalui Meta Graph API rasmi sementara menunggu rollout MCP.";
}
...
async function fetchMetaDailyRawFromMcp(accountId, reportDate) {
...
async function fetchMetaDailyRaw(accountId, reportDate) {
  try {
    return await fetchMetaDailyRawFromMcp(accountId, reportDate);
  } catch (error) {
    if (!isMcpAccountUnavailable(error)) throw error;
    const { state } = await loadStoredConnection();
    const fields = "spend,impressions,clicks,inline_link_clicks,reach,frequency,cpm,cpc,ctr,actions,action_values,purchase_roas";
    const [accountRows, campaignRows] = await Promise.all([
      metaGraphInsights(accountId, { fields, level: "account", time_range: { since: reportDate, until: reportDate }, limit: 10 }, state),
      metaGraphInsights(accountId, { fields: `campaign_id,campaign_name,${fields}`, level: "campaign", time_range: { since: reportDate, until: reportDate }, limit: 500 }, state),
    ]);
    return {
      account: accountRows[0] ? normalizeGraphRow(accountRows[0], "account") : null,
      campaigns: campaignRows.map((row) => normalizeGraphRow(row, "campaign")),
...
function aggregateMetaDailyRaw(raw, configInput = {}) {
  ...
  if (Array.isArray(raw?.warnings) && raw.warnings.length) analytics.warnings.unshift(...raw.warnings);
  return analytics;
}
...
async function fetchMetaRangeFromMcp(configInput, startDate, endDate, { full = true } = {}) {
  const config = normalizeAdsReportConfig(configInput);
  if (!config.accountId) throw new Error("Pilih Meta ad account dahulu.");
...
async function fetchMetaRangeFromGraph(configInput, startDate, endDate, { full = true } = {}) {
  const config = normalizeAdsReportConfig(configInput);
  ...
  const definitions = [
    ...
    ...(full ? [
      ["adset", `adset_id,adset_name,campaign_id,campaign_name,${base}`],
      ["ad", `ad_id,ad_name,adset_id,adset_name,campaign_id,campaign_name,${base}`],
    ] : []),
  ];
  const byLevel = await fetchGraphLevels(config, startDate, endDate, definitions);
  const insights = { ...(byLevel.account?.[0] ? normalizeGraphRow(byLevel.account[0], "account") : {}), currency: config.currency };
  const analytics = aggregateMetaData({
    insights,
    campaigns: (byLevel.campaign || []).map((row) => normalizeGraphRow(row, "campaign")),
    adsets: (byLevel.adset || []).map((row) => normalizeGraphRow(row, "adset")),
    ads: (byLevel.ad || []).map((row) => normalizeGraphRow(row, "ad")),
  }, config);
  analytics.startDate = startDate;
  analytics.endDate = endDate;
  analytics.source = "meta_graph_fallback";
  analytics.warnings.unshift(fallbackWarning());
  if (!byLevel.account?.length && !byLevel.campaign?.length) analytics.warnings.push("Meta Graph API tidak memulangkan insights untuk tempoh ini.");
  return analytics;
}

async function fetchMetaRange(configInput, startDate, endDate, options = {}) {
  try {
    return await fetchMetaRangeFromMcp(configInput, startDate, endDate, options);
  } catch (error) {
    if (!isMcpAccountUnavailable(error)) throw error;
    return fetchMetaRangeFromGraph(configInput, startDate, endDate, options);
  }
}

async function fetchMetaCustomReportFromMcp(configInput, startDate, endDate) {
  const config = normalizeAdsReportConfig(configInput);
  if (!config.accountId) throw new Error("Pilih Meta ad account dahulu.");
...
async function fetchMetaCustomReport(configInput, startDate, endDate) {
  try {
    return await fetchMetaCustomReportFromMcp(configInput, startDate, endDate);
  } catch (error) {
    if (!isMcpAccountUnavailable(error)) throw error;
    validateCustomWeek(startDate, endDate);
    return fetchMetaRangeFromGraph(configInput, startDate, endDate, { full: true });
  }
}
...
module.exports = {
  ...
  validateCustomWeek,
  _test: { decodeMaybeJson, isMcpAccountUnavailable, normalizeMetaAccount, rowsFromPayload, schemaArgument },
};
