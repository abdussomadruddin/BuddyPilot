...
function assessPeriod(analytics, setting) {
  ...
  if (!setting.productRules.length) warnings.push("Product profitability rules belum dikonfigurasi; status untung/rugi tidak tersedia.");
  const uniqueWarnings = [...new Set(warnings)];
  return {
    ...analytics,
    campaigns,
    profitability: {
      contributionProfit: coveredSpend > 0 ? round(contributionProfit) : null,
