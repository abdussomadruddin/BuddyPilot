const { fetchMetaRange, listMetaAdAccounts } = require("./meta-ads");
...
function buildProductBreakdown(campaigns = []) {
  ...
  for (const campaign of campaigns) {
    ...
    const current = groups.get(product) || {
      ...
      profitCampaigns: 0, campaignCount: 0, profitSources: new Set(),
    };
    ...
    if (campaign.profitSource) current.profitSources.add(campaign.profitSource);
  ...
  return [...groups.values()].map((item) => ({
    ...
    profit: item.profitCampaigns ? round(item.profit) : null,
    profitComplete: item.profitCampaigns === item.campaignCount,
    profitSource: [...item.profitSources].join(", "),
    purchases: round(item.purchases),
    cpp: item.purchases > 0 ? round(item.spend / item.purchases) : null,
    roas: item.spend > 0 ? round(item.revenue / item.spend) : null,
...
function keywordMatches(name, keyword) {
  ...
  const needle = normalizedWords(keyword);
  if (!needle) return false;
  if ((` ${haystack} `).includes(` ${needle} `)) return true;
  const compactNeedle = needle.replace(/\s+/g, "");
  return compactNeedle.length >= 4 && haystack.replace(/\s+/g, "").includes(compactNeedle);
}

function matchingRule(name, rules) {
  const matches = [];
  for (const rule of rules) {
    for (const keyword of rule.campaignKeywords) {
      if (keywordMatches(name, keyword)) matches.push({ rule, keyword: normalizedWords(keyword) });
    }
  }
  matches.sort((a, b) => b.keyword.length - a.keyword.length || a.rule.name.localeCompare(b.rule.name));
  return matches[0]?.rule || null;
}

function metricTotals(campaigns = []) {
  return campaigns.reduce((total, campaign) => {
    for (const key of ["spend", "revenue", "purchases", "leads", "messaging", "impressions", "reach", "clicks", "linkClicks"]) {
      total[key] += number(campaign[key]);
    }
    return total;
  }, { spend: 0, revenue: 0, purchases: 0, leads: 0, messaging: 0, impressions: 0, reach: 0, clicks: 0, linkClicks: 0 });
}

function accountOnlyCampaign(total = {}, campaigns = [], setting = {}) {
  const summed = metricTotals(campaigns);
  const residual = {};
  let hasResidual = false;
  for (const key of Object.keys(summed)) {
    const difference = round(Math.max(0, number(total[key]) - summed[key]));
    residual[key] = difference;
    if (difference > 0.01) hasResidual = true;
  }
  if (!hasResidual) return null;
  const onlyRule = setting.productRules?.length === 1 ? setting.productRules[0] : null;
  let product = "Account-only / Unattributed";
  let primaryMetric = "";
  let primaryResultCount = 0;
  let revenue = residual.revenue;
  let profit = round(revenue - residual.spend);
  let profitSource = revenue > 0 ? "reported_revenue_less_ads" : "spend_only";
  if (onlyRule) {
    product = onlyRule.name;
    primaryMetric = onlyRule.primaryMetric;
    primaryResultCount = primaryResults(residual, primaryMetric);
    if (primaryMetric === "purchase" && !revenue && primaryResultCount > 0 && onlyRule.sellingPrice > 0) {
      revenue = round(primaryResultCount * onlyRule.sellingPrice);
      profitSource = "estimated_revenue_margin";
    }
    if (revenue > 0) {
      const margin = onlyRule.grossMarginPercent > 0 ? onlyRule.grossMarginPercent / 100 : 1;
      profit = round(revenue * margin - residual.spend);
      if (profitSource !== "estimated_revenue_margin") profitSource = onlyRule.grossMarginPercent > 0 ? "reported_revenue_margin" : "reported_revenue_less_ads";
    } else if (onlyRule.allowableCpa > 0 && primaryResultCount > 0) {
      profit = round(primaryResultCount * onlyRule.allowableCpa - residual.spend);
      profitSource = "allowable_cpa_headroom";
    }
  }
  return {
    id: "account-only-unattributed",
    name: "Account totals not attributed to campaigns",
    status: "",
    category: "other",
    product,
    primaryMetric,
    primaryResults: primaryResultCount,
    primaryCpa: primaryResultCount > 0 ? round(residual.spend / primaryResultCount) : null,
    allowableCpa: onlyRule?.allowableCpa || null,
    ...residual,
    revenue,
    profit,
    profitSource,
...
function assessPeriod(analytics, setting) {
  ...
  const campaigns = analytics.campaigns.map((campaign) => {
    ...
    if (!rule) {
      ...
      const profit = round(revenue - number(campaign.spend));
      return {
        ...campaign,
        product: "Other / Unmapped",
        primaryMetric: "",
        primaryResults: 0,
        primaryCpa: null,
        allowableCpa: null,
        profit,
        profitSource: revenue > 0 ? "reported_revenue_less_ads" : "spend_only",
        profitStatus: "unmapped",
      };
    }
    ...
    if (rule.primaryMetric === "purchase") {
      ...
      if (rule.grossMarginPercent > 0 && revenue > 0 && !anomaly) {
        profit = round(revenue * (rule.grossMarginPercent / 100) - campaign.spend);
        profitSource = revenueSource === "estimated" ? "estimated_revenue_margin" : "reported_revenue_margin";
      } else if (!anomaly) {
        profit = round(revenue - campaign.spend);
        profitSource = revenue > 0 ? "reported_revenue_less_ads" : "spend_only";
      } else {
        profit = round(revenue - campaign.spend);
        profitSource = "tracking_fallback";
      }
    } else if (revenue > 0) {
      const margin = rule.grossMarginPercent > 0 ? rule.grossMarginPercent / 100 : 1;
      profit = round(revenue * margin - campaign.spend);
      profitSource = rule.grossMarginPercent > 0 ? "reported_revenue_margin" : "reported_revenue_less_ads";
  ...
  const residualCampaign = accountOnlyCampaign(analytics.total, campaigns, setting);
  if (residualCampaign) {
    campaigns.push(residualCampaign);
    warnings.push("Sebahagian account totals tidak mempunyai pecahan campaign; dipaparkan sebagai Account-only / Unattributed.");
  }
  const unmappedSpend = round(campaigns.filter((item) => item.profitStatus === "unmapped").reduce((sum, item) => sum + item.spend, 0));
  if (unmappedSpend > 0) warnings.push(`${money(unmappedSpend, setting.currency)} spend berada dalam Other / Unmapped; profit bahagian ini menggunakan reported revenue − ads spent.`);
  if (!setting.productRules.length) warnings.push("Product profitability rules belum dikonfigurasi; Est. profit menggunakan reported revenue − ads spent.");
  let accountProfit = campaigns.every((item) => item.profit != null)
    ? round(campaigns.reduce((sum, item) => sum + number(item.profit), 0))
    : null;
  const profitSources = [...new Set(campaigns.map((item) => item.profitSource).filter(Boolean))];
  let profitSource = profitSources.length === 1 ? profitSources[0] : profitSources.length ? "mixed" : "unavailable";
  if (accountProfit == null) {
    ...
    warnings.push("Est. profit menggunakan reported revenue − ads spent untuk bahagian yang product mapping atau margin belum lengkap.");
  }
  ...
  return {
    ...
    profitability: {
      ...
      coveragePercent: analytics.total.spend > 0 ? round((Math.min(coveredSpend, analytics.total.spend) / analytics.total.spend) * 100, 1) : null,
      estimatedRevenue: round(campaigns.reduce((sum, item) => sum + number(item.revenue), 0) || estimatedRevenue || analytics.total.revenue),
      unmappedSpend,
    },
...
function buildLiveSnapshot(analytics, setting, { capturedAt = new Date().toISOString(), reportDate = todayDate() } = {}) {
  ...
  return {
    ...
    campaigns: assessed.campaigns.map((campaign) => ({
      ...
      revenue: round(campaign.revenue),
      profit: campaign.profit == null ? null : round(campaign.profit),
      purchases: round(campaign.purchases),
      cpp: number(campaign.purchases) > 0 ? round(number(campaign.spend) / number(campaign.purchases)) : null,
      roas: number(campaign.spend) > 0 ? round(number(campaign.revenue) / number(campaign.spend)) : null,
...
function metricComparison(current, previous) {
  ...
  const currentProfit = current.profitability?.contributionProfit ?? null;
  const previousProfit = previous.profitability?.contributionProfit ?? null;
  return [
    ...
    { metric: "Est. Profit", current: currentProfit, previous: previousProfit, differencePercent: currentProfit == null || previousProfit == null ? null : pctChange(currentProfit, previousProfit), format: "money" },
...
async function listAccountsWithSettings() {
  const [live, stored] = await Promise.all([listMetaAdAccounts(), listPersonalAdsAccounts()]);
...
async function getPersonalAdsLiveSnapshot(accountInput, { now = new Date() } = {}) {
  ...
  const analytics = await fetchMetaRange(config, reportDate, reportDate, { full: true });
...
async function generatePersonalAdsReport(accountInput, reportDate, { retry = false } = {}) {
  ...
  if (existing?.status === "ready" && !retry) return existing;
  ...
  try {
    ...
    const [yesterdayRaw, currentRaw, comparisonRaw] = await Promise.all([
      fetchMetaRange(config, reportDate, reportDate, { full: true }),
      fetchMetaRange(config, currentStart, reportDate, { full: true }),
      fetchMetaRange(config, comparisonStart, comparisonEnd, { full: false }),
    ]);
    if (!yesterdayRaw.total.spend && !yesterdayRaw.campaigns.length) throw new Error("Meta tidak memulangkan insights semalam; report tidak dijana sebagai angka sifar.");
...
function recalculateStoredReport(report, accountInput = {}) {
  if (!report?.yesterday) return report;
  const setting = normalizeAccountSetting(accountInput);
  const yesterday = assessPeriod(report.yesterday, setting);
  const currentPeriod = report.current_period ? assessPeriod(report.current_period, setting) : yesterday;
  const comparisonPeriod = report.comparison_period ? assessPeriod(report.comparison_period, setting) : currentPeriod;
  return {
    ...report,
    yesterday,
    current_period: currentPeriod,
    comparison_period: comparisonPeriod,
    diagnosis: diagnose(yesterday, currentPeriod, comparisonPeriod, setting),
  };
}
