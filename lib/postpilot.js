...
async function publishToFacebook({ file, caption, firstComment }) {
  ...
  let mediaResponse;
  if (mediaType === "video") {
    const videoDescription = firstComment
      ? `${caption}\n\n${firstComment}`
      : caption;
  ...
  const fallbackVideoUrl = mediaType === "video" && mediaResponse.id
    ...
    : "";
  const permalinkUrl = normalizeFacebookUrl(videoDetails.permalink_url || await fetchPermalinkSafe(postId, pageAccessToken) || fallbackVideoUrl);
  let commentResponse = null;
  let commentNote = "";
  if (firstComment && mediaType === "video") {
    commentNote = "Untuk video/reel, CTA first comment dimasukkan terus dalam description kerana Graph API comments endpoint untuk object video ini deprecated.";
  } else if (firstComment && postId) {
