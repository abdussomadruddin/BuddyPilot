...
async function listPostPilotHookImagesFromManifest() {
  ...
  let images = draft.hookGalleryImages;
  // Recover image objects uploaded before the gallery table existed. The bucket
  // remains image-only; metadata is written back to the existing draft row.
  if (!images.length) {
    ...
    images = (Array.isArray(objects) ? objects : [])
      .filter((object) => object?.name && !/\.json$/i.test(object.name))
      .map((object) => normalizeManifestHookImage({
        id: `storage-${Buffer.from(`gallery/${object.name}`).toString("base64url")}`,
        storagePath: `gallery/${object.name}`,
        name: object.name.replace(/^hook-[^-]+-[^-]+\./, "hook."),
        type: object.metadata?.mimetype || "image/jpeg",
        size: object.metadata?.size || 0,
        createdAt: object.created_at || object.updated_at,
      }));
    if (images.length) await upsertPostPilotDraft({ hookGalleryImages: images });
  }
...
async function uploadPostPilotHookGalleryImage(file) {
  ...
  try {
    ...
    if (isMissingSupabaseTableError(error)) {
      const { hookGalleryImages: images } = await getPostPilotDraft();
      ...
      await upsertPostPilotDraft({ hookGalleryImages: [...images, image] });
      return image;
    }
...
async function deletePostPilotHookGalleryImage(id) {
  ...
  try {
    ...
    if (!isMissingSupabaseTableError(error)) throw error;
    const { hookGalleryImages: images } = await getPostPilotDraft();
    await upsertPostPilotDraft({ hookGalleryImages: images.filter((item) => item.id !== id) });
  }
  return image;
}
...
async function reservePostPilotHookImages(count) {
  ...
  try {
    ...
    const updated = all.map((image) => selectedIds.has(image.id)
      ...
      : image);
    await upsertPostPilotDraft({ hookGalleryImages: updated });
  }
