...
async function getCopyPilotSettings() {
  const rows = await supabaseRequest("operations_health?service_name=eq.copy_pilot_secret&select=metadata&limit=1");
  return rows?.[0]?.metadata || null;
}

async function saveCopyPilotSettings(value = {}) {
  const rows = await supabaseRequest("operations_health?on_conflict=service_name", {
    method: "POST",
    prefer: "resolution=merge-duplicates,return=representation",
    body: [{
      service_name: "copy_pilot_secret",
      status: "healthy",
      detail: "Copy Pilot key configured.",
      metadata: {
        encrypted_openai_key: String(value.encryptedOpenaiKey || ""),
        key_hint: String(value.keyHint || ""),
      },
