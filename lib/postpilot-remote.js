...
function jobToPublic(row) {
  ...
  return {
    ...
    recovery: recoveryAttempt ? {
      ...
      recovered: row.status === "completed",
    } : null,
...
function deviceToPublic(row, activeJob = null) {
  ...
  return {
    ...
    status: activeJob ? "busy" : lastSeenMs <= DEVICE_ONLINE_MS ? "online" : "offline",
...
async function heartbeatDevice(device) {
  ...
  await reportOperationalSuccess({
    fingerprints: [`service:mac_extension`, `mac-offline-queue:${device.id}`],
    serviceName: "mac_extension",
    detail: "Chrome Mac online dan sedia menerima job.",
    metadata: { deviceId: device.id, lastSeenAt: now },
  });
}
...
function validatePayload(type, payload) {
  ...
  for (const post of posts) {
    const caption = type === "facebook_threads"
      ? post?.facebookPostText || post?.postText
      : post?.postText;
    if (!String(caption || "").trim()) throw Object.assign(new Error("Caption post tidak boleh kosong."), { statusCode: 400 });
...
async function createJob({ type, payload, device: providedDevice = null }) {
  ...
  const deviceOffline = !device.last_seen_at || Date.now() - new Date(device.last_seen_at).getTime() > DEVICE_ONLINE_MS;
  if (deviceOffline) {
...
async function finishJob(device, { jobId, status, error = "", progress = {} }) {
  ...
  if (finalStatus === "completed" && job.job_type === "threads_text") {
    const historyIds = (job.payload?.posts || []).map((post) => post.id).filter(Boolean);
    await markPostPilotCopyHistoryPublished(historyIds, now).catch(() => []);
  }
