...
async function adflowCall(name, args = {}) {
  return withTransientRetry(async () => {
    ...
    if (!response.ok || json?.error) {
      const error = new Error(`AdFlow MCP gagal: ${json?.error?.message || `HTTP ${response.status}`}`);
...
function normalizeGraphRow(row, level) {
  const conversions = firstActionValue(row.actions, ["purchase", "omni_purchase", "offsite_conversion.fb_pixel_purchase", "onsite_web_purchase"]);
  const leads = firstActionValue(row.actions, ["lead", "onsite_conversion.lead_grouped", "offsite_conversion.fb_pixel_lead"]);
  const conversations = firstActionValue(row.actions, [
    "onsite_conversion.messaging_conversation_started_7d",
    "messaging_conversation_started_7d",
    "onsite_conversion.total_messaging_connection",
  ]);
  const revenue = firstActionValue(row.action_values, ["purchase", "omni_purchase", "offsite_conversion.fb_pixel_purchase", "onsite_web_purchase"]);
  const common = {
...
async function fetchAdflowRange(configInput, startDate, endDate, { full = true } = {}) {
  const config = normalizeAdsReportConfig(configInput);
  if (!config.accountId) throw new Error("Pilih AdFlow ad account dahulu.");
  ...
  if (!byLevel.account?.data?.length && !byLevel.campaign?.data?.length) analytics.warnings.unshift("AdFlow tidak memulangkan insights untuk tempoh ini.");
  return analytics;
}
