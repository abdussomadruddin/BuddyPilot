...
const { listMetaAdAccounts, metaConnectionStatus } = require("./meta-ads");
const {
  listActivity,
...
const SERVICE_DEFINITIONS = [
  ...
  { id: "meta_mcp", label: "Meta MCP", description: "Meta Ads reporting", action: { kind: "href", label: "Connect", href: "/api/meta/oauth-start" } },
...
function serviceContext(clients = [], remote = {}) {
  ...
  return {
    ...
    meta_mcp: { configured: Boolean(process.env.META_TOKEN_ENCRYPTION_KEY || process.env.TIKTOK_TOKEN_ENCRYPTION_KEY), required: metaUsed },
...
function buildOperationsOverview({ clients = [], remote = {}, tiktok = {}, healthRows = [], incidentRows = [], deliveries = [], activities = [], warnings = [], now = new Date() } = {}) {
  ...
  const healthyServices = new Set(health.filter((item) => item.status === "healthy").map((item) => item.id));
  const incidents = mergeIncidents(incidentRows, syntheticIncidents(remote, deliveries, names), names)
    .filter((item) => !item.service || !healthyServices.has(item.service));
  const requiredProblems = health.filter((item) => item.required && item.status !== "healthy");
  const incidentServices = new Set(incidents.map((item) => item.service).filter(Boolean));
  const standaloneHealthProblems = requiredProblems.filter((item) => !incidentServices.has(item.id));
  ...
  return {
    ...
    summary: {
      ...
      attention: incidents.length + standaloneHealthProblems.length,
...
async function probeService(serviceName, remote) {
  ...
  if (serviceName === "meta_mcp") {
    const connection = await metaConnectionStatus();
    if (!connection.connected) return { status: connection.status === "expired" ? "down" : "setup", detail: connection.error || "Meta MCP belum connected.", metadata: connection };
    const accounts = await listMetaAdAccounts();
    return { status: connection.status === "expiring" ? "warning" : "healthy", detail: `${accounts.length} Meta ad account tersedia.`, metadata: { ...connection, accountCount: accounts.length } };
  }
