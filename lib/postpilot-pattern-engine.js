...
const THREADS_CLAUSES = Object.freeze({
  ...
  directQuestion: [
    ...
    "ada siapa pernah stop {topic}, lepas tu start balik?",
  ],
  softClose: [
...
function composeThreadsClauses({ family, topic, rng }) {
  ...
  if (family === "story") return [from("personalOpen"), `${topic} buat aku ${from("tension")}`, from("realization")];
  if (family === "opinion") return [`${from("personalOpen")}, ${topic} tak perlu dibuat susah sangat`, from("realization")];
  if (family === "insight") return [from("crowdOpen"), from("tension"), from("realization")];
  if (family === "recommendation") return [`kalau baru nak cuba ${topic}, jangan ${from("tension")}`, from("softClose")];
...
function applyMalaysiaThreadsStyle(text, { family, tone, rng }) {
  ...
  if (!["list", "story"].includes(family) && rng() < 0.38) {
